...
void img_check_pan(img_t *img, bool moved)
{
	...
	if (w < win->width)
		...
		img->x = 0;
	else if (img->x + w < win->width)
		img->x = win->width - w;
	if (h < win->height)
		img->y = (win->height - h) / 2;
	else if (img->y > 0)
		img->y = 0;
	else if (img->y + h < win->height)
		img->y = win->height - h;
...
bool img_fit(img_t *img)
{
	...
	zw = (float) img->win->width / (float) img->w;
	zh = (float) img->win->height / (float) img->h;
...
void img_render(img_t *img)
{
	...
	if (img->y <= 0) {
		...
		// might be a pixel short when using the int truncation in C.
		dh = (int)round((double)img->h * (double)img->zoom);
	}
	...
	bool has_alpha = imlib_image_has_alpha();
	...
	if (has_alpha && img->alpha) {
		...
		// set alpha to 0xFF, otherwise imlib_blend_image_onto_image won't work
		memset(data, 0xFF, dh * dw * 4);
	}
	...
	win_render_imlib_image(win, dx, dy);
...
bool img_fit_win(img_t *img, scalemode_t sm)
{
	...
	if (img_fit(img)) {
		img->x = img->win->width / 2 - (img->win->width / 2 - img->x) * img->zoom / oz;
		img->y = img->win->height / 2 - (img->win->height / 2 - img->y) * img->zoom / oz;
		img->checkpan = true;
		return true;
	} else {
		return false;
...
bool img_zoom(img_t *img, float z)
{
	...
	if (zoomdiff(img, z) != 0) {
		...
		if (x < 0 || x >= img->win->width || y < 0 || y >= img->win->height) {
			x = img->win->width / 2;
			y = img->win->height / 2;
		}
...
bool img_pan(img_t *img, direction_t dir, int d)
{
	...
	if (d > 0) {
		...
		x = img->win->width / (d < 0 ? 1 : PAN_FRACTION);
		y = img->win->height / (d < 0 ? 1 : PAN_FRACTION);
	}
...
bool img_pan_edge(img_t *img, direction_t dir)
{
	...
	if (dir & DIR_LEFT)
		img->x = 0;
	if (dir & DIR_RIGHT)
		img->x = img->win->width - img->w * img->zoom;
	if (dir & DIR_UP)
		img->y = 0;
	if (dir & DIR_DOWN)
		img->y = img->win->height - img->h * img->zoom;
...
void img_rotate(img_t *img, degree_t d)
{
	...
	if (d == DEGREE_90 || d == DEGREE_270) {
		ox = d == DEGREE_90  ? img->x : img->win->width - img->x - img->w * img->zoom;
		oy = d == DEGREE_270 ? img->y : img->win->height - img->y - img->h * img->zoom;

		img->x = oy + (img->win->width - img->win->height) / 2;
		img->y = ox + (img->win->height - img->win->width) / 2;

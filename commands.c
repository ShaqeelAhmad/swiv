...
bool ci_drag(arg_t mode)
{
	...
	if ((int)(img.w * img.zoom) <= win.width && (int)(img.h * img.zoom) <= win.height)
		return false;
	...
	if (mode == DRAG_ABSOLUTE) {
		px = MIN(MAX(0.0, x - win.width*0.1), win.width*0.8) / (win.width*0.8)
			* (win.width - img.w * img.zoom);
		py = MIN(MAX(0.0, y - win.height*0.1), win.height*0.8) / (win.height*0.8)
			* (win.height - img.h * img.zoom);
	} else {
		px = img.x + x - ox;
		py = img.y + y - oy;
	}
